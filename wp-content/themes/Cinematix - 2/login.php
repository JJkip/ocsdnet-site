...
<?php get_header(); ?>
	...
	<?php else : ?>
		...
		<form name="login-form" id="page-login-form" class="standard-form" action="<?php echo site_url( 'wp-login.php', 'login_post' ) ?>" method="post">
			<label><?php _e( 'Username', 'Cinematix' ) ?><br />
			...
			<label><?php _e( 'Password', 'Cinematix' ) ?><br />
			<input type="password" name="pwd" id="page-user-pass" class="input" value="" tabindex="98" /></label>

			<p class="forgetmenot"><label><input name="rememberme" type="checkbox" id="sidebar-rememberme" value="forever" tabindex="99" /> <?php _e( 'Remember Me', 'Cinematix' ) ?> / <a href="<?php echo home_url(); ?>/<?php _e('recovery', 'Cinematix'); ?>"><?php _e('Password Recovery', 'Cinematix'); ?></a></label></p>

			<?php do_action( 'bp_sidebar_login_form' ) ?>
			<input type="submit" name="wp-submit" id="wp-submit" value="<?php _e( 'Log In', 'Cinematix' ); ?>" tabindex="100" />
		</form>
