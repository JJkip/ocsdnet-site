...
{if($.fn.superfish){$('.base-menu ul.sf-menu,.bp-user-nav').superfish({delay:400,animation:{opacity:'show',height:'show'},speed:'fast',autoArrows:false,dropShadows:false,onShow:function(){$(this).css('overflow','visible');}});}
$('.comment-reply-link').addClass('button');$('.register #content').addClass('column sixteen');infinity_sidebar_height();$('#sidebar').bind('ajaxStop',function(){infinity_sidebar_height();});});function infinity_sidebar_height()
{var height=jQuery('.main-wrap').height();jQuery('#sidebar').css('min-height',height);}
